...
113	moenv_wr_recycle_6a4bc66658	家具類	circle	geojson	small	\N	{"circle-color":"#56B96D","circle-opacity":0.85,"circle-radius":2}	[{"key":"site_name","name":"點位名稱"},{"key":"full_address","name":"地址"},{"key":"item","name":"回收類別"},{"key":"service_type","name":"服務類型"},{"key":"tel","name":"電話"}]
114	moenv_wr_recycle_014ce45e54	書籍類	circle	geojson	small	\N	{"circle-color":"#F8CF58","circle-opacity":0.85,"circle-radius":2}	[{"key":"site_name","name":"點位名稱"},{"key":"full_address","name":"地址"},{"key":"item","name":"回收類別"},{"key":"service_type","name":"服務類型"},{"key":"tel","name":"電話"}]
115	moenv_wr_recycle_8863327e9b	玩具類	circle	geojson	small	\N	{"circle-color":"#F5AD4A","circle-opacity":0.85,"circle-radius":2}	[{"key":"site_name","name":"點位名稱"},{"key":"full_address","name":"地址"},{"key":"item","name":"回收類別"},{"key":"service_type","name":"服務類型"},{"key":"tel","name":"電話"}]
...
360	garbage_map_metrotaipei	垃圾地圖	{219,220,999,229,300,301,303}	delete	2026-05-02 00:00:00+00	2026-05-02 00:00:00+00
...
incinerator_capacity	\N	{103}	{"mode":"byParam","byParam":{"xParam":"district"}}	static	\N	0	\N	環保局	顯示雙北焚化爐處理量能。	此圖顯示雙北地區各焚化廠的處理量能比率，以行政區為單位呈現，顏色由綠色（低負載）過渡至紅色（高負載），可直觀了解各焚化廠的負荷狀況。資料來源為 2026 年 2 月份雙北焚化廠實際處理量與設計容量比對結果。	可用於評估雙北垃圾焚化設施的容量使用狀況，協助環保主管機關掌握各廠的運作負荷，作為廢棄物處理政策與設施擴充規劃的重要參考依據。	{https://data.moenv.gov.tw/dataset/detail/FAC_S_02}	{doit,ntpc}	2026-05-02 00:00:00+00	2026-05-02 00:00:00+00	three_d	select district as x_axis, '焚化爐處理量能' as y_axis, round(capacity_ratio) as data from (values ('北投區','臺北市北投垃圾焚化廠',68.84),('文山區','臺北市木柵垃圾焚化廠',58.39),('內湖區','臺北市內湖垃圾焚化廠',52.86),('八里區','新北市八里垃圾焚化廠',89.97),('新店區','新北市新店垃圾焚化廠',69.56),('樹林區','新北市樹林垃圾焚化廠',62.04)) as t(district,name,capacity_ratio)	\N	metrotaipei
moenv_wr_recycle_metrotaipei	\N	{111,112,113,114,115,116,117,118}	{}	static	\N	0	\N	環境部	北北基地區環境部公開回收點，八類物資分層顯示。	資料來自環境部回收點位公開資訊；範圍為臺北市、新北市、基隆市（已排除桃園市）。於組件內可獨立開關各類別圖層。	先開啟組件地圖開關，再依需求勾選要顯示的物資類別。	{https://data.gov.tw/dataset/163144}	{doit,ntpc}	2026-05-02 00:00:00+00	2026-05-02 00:00:00+00	three_d	select * from (values ('MOENV 回收點','圖層數','類',8),('MOENV 回收點','涵蓋縣市','項',3)) as t(x_axis,y_axis,icon,data)	\N	metrotaipei
ntpc_recycling_map_mvp	\N	{150}	{"mode":"byParam","byParam":{"xParam":"TNAME"}}	static	\N	0	\N	新北市政府開放資料（整理）	新北市各行政區資源回收量（區小計／總計），單位：公噸（由公斤換算）。資料期別：中華民國115年3月。	MVP 為單月快照；臺北市區域於圖資上無數值（0）。之後可擴充多月與時間序列組件（history_config / query_history）。	用於觀察各區資源回收量與空間分布。	{https://data.gov.tw/dataset/163144,https://oas.bas.ntpc.gov.tw/NTPCTRWD/NewPage/Publish.aspx?Mid1=382150000I&p=2&y=2025/12/25&s=10}	{ntpc}	2026-05-02 00:00:00+00	2026-05-02 00:00:00+00	three_d	SELECT district AS x_axis,\r\n       '資源回收量(公噸)' AS y_axis,\r\n       ''::text AS icon,\r\n       round(recycling_kg::numeric / 1000)::int AS data\r\nFROM public.ntpc_waste_mvp\r\nORDER BY 1	\N	metrotaipei
ntpc_kitchen_waste_map_mvp	\N	{151}	{"mode":"byParam","byParam":{"xParam":"TNAME"}}	static	\N	0	\N	新北市政府開放資料（整理）	新北市各行政區廚餘回收量，單位：公噸（由原月報公噸換算後以整數公噸呈現）。資料期別：中華民國115年2月。	MVP 為單月快照；烏來區原表缺值時為 0。之後可擴充歷史月並改用 time 類型查詢。	用於觀察各區廚餘回收量與空間分布。	{https://oas.bas.ntpc.gov.tw/NTPCTRWD/NewPage/Publish.aspx?Mid1=382150000I&p=2&y=2025/12/25&s=10}	{ntpc}	2026-05-02 00:00:00+00	2026-05-02 00:00:00+00	three_d	SELECT district AS x_axis,\r\n       '廚餘回收量(公噸)' AS y_axis,\r\n       ''::text AS icon,\r\n       round(kitchen_kg::numeric / 1000)::int AS data\r\nFROM public.ntpc_waste_mvp\r\nORDER BY 1	\N	metrotaipei
\.
