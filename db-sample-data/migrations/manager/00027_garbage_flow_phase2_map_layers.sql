...
-- 新增清運收運流向第二階段弧線圖層（集中點 → 焚化廠）
-- component_maps id 323: garbage_ntpc_hub_incinerator_arcs_local（新北）
-- component_maps id 324: garbage_taipei_hub_incinerator_arcs_local（臺北）
-- 並更新 garbage_district_compare_local 之 map_config_ids 以包含這兩層。
...
-- 2. 更新 garbage_district_compare_local（metrotaipei）的 map_config_ids，加入 323、324
UPDATE query_charts
...
-- 3. 更新 garbage_district_compare_local（taipei）的 map_config_ids，加入 324（臺北段）
UPDATE query_charts
...
UPDATE query_charts
SET map_config_ids = ARRAY[320, 321]::integer[],
    updated_at = NOW()
WHERE index = 'garbage_district_compare_local'
  AND city = 'metrotaipei';

UPDATE query_charts
SET map_config_ids = ARRAY[321]::integer[],
    updated_at = NOW()
WHERE index = 'garbage_district_compare_local'
  AND city = 'taipei';
