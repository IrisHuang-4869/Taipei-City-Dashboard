...
-- source: db-sample-data/dashboardmanager-demo.sql

-- +goose Up
-- 清空所有表，讓 COPY 可重複執行（僅首次 migration 需要）
TRUNCATE TABLE
  public.dashboard_groups, public.dashboards, public.query_charts,
  public.component_charts, public.component_maps, public.components,
  public.groups, public.contributors, public.issues
...
-- +goose Down
-- 無法安全回滾整個 demo dump；如需還原請重新套用完整 migration
